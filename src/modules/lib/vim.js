...
var merge_dict = function (dict_a, dict_b) {
  ...
  var clone = function (src) {
    for (var p in src) if (src.hasOwnProperty(p)) r[p] = src[p];
  };
    ...
    if (x === undefined) {
      ...
      if (this.m_cnode.is_leaf()) {
        ...
        if (this.m_cdata.digit === undefined) {
          nrepeats =
            this.m_digit_buffer === "" ? 1 : parseInt(this.m_digit_buffer);
          nrepeats =
            nrepeats > NREPEATS_LIMIT
              ? NREPEATS_LIMIT
              : nrepeats < 1
              ? 1
              : nrepeats;
...
var build_tree_command_mode = function () {
  ...
  var choices_ai = {
    a: node()
      .set_choice(
        "p",
        node({ select_func: d_with_whitespaces_after(select_paragraph) })
      )
      .set_choice("w", node({ select_func: find_word_with_spaces_after }))
      .set_choice(
        "W",
        node({ select_func: d_with_spaces_after(find_word_plus) })
      )
      .set_choice(
        "'",
        node({ select_func: d_in_line(select_quotes.partial("'")) })
      )
      .set_choice(
        '"',
        node({ select_func: d_in_line(select_quotes.partial('"')) })
    ...
    i: node()
      ...
      .set_choice(
        "'",
        node({ select_func: d_inside(d_in_line(select_quotes.partial("'"))) })
      )
      .set_choice(
        '"',
        node({ select_func: d_inside(d_in_line(select_quotes.partial('"'))) })
      ...
      .set_choice(
        "(",
        node({ select_func: d_inside(select_bounds.partial("()")) })
      )
      .set_choice(
        ")",
        node({ select_func: d_inside(select_bounds.partial("()")) })
...
var build_tree_visual_mode = function () {
  var _r = node()
    ...
    .set_choice(
      "d",
      node({ action: act_delete_current_selection, mode: COMMAND })
    )
    .set_choice(
      "c",
      node({ action: act_delete_current_selection, mode: INSERT })
    );
  return _r;
};
...
var make_choices_for_navigation = function (params) {
  ...
  var N = function (d) {
    return node(merge_dict(params, d));
  };
...
var cut_slice = function (text, i0, i1) {
  ...
  return {
    text: text.substr(0, from) + text.substr(to),
...
var act_indent_increase = function (vim, cdata) {
  ...
  __alter_selection(vim, cdata, function (t) {
    return t.replace(/^/gm, " ");
  });
};
