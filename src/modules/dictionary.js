...
export function Dictionary() {
  ...
  this.build_synonyms = function () {
    ...
    for (const targetWord in SYN_DB) {
      ...
      for (const wordID in synonyms) {
        ...
        if (
          this.synonyms[targetParent] &&
          this.synonyms[targetParent].constructor === Array
        ) {
          this.synonyms[targetParent][this.synonyms[targetParent].length] =
            targetWord;
        } else {
    ...
    console.log(
      `Built ${Object.keys(this.synonyms).length} synonyms, in ${(
  ...
  this.update = function () {
    ...
    console.log(
      `Updated Dictionary in ${(performance.now() - time).toFixed(2)}ms.`
    );
  };
